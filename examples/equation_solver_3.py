from typing import Optional

from pyguiadapterlite import GUIAdapter, ParameterError, uprint, FnExecuteWindowConfig
from pyguiadapterlite.types import float_r


def equation_solver_3(
    a: float_r = 1.0, b: float_r = 0.0, c: float_r = 0.0
) -> Optional[tuple]:
    """A simple equation solver for equations like:

    ax^2 + bx + c = 0 (a, b, c ∈ R and a ≠ 0)

    @param a: a ∈ R and a ≠ 0
    @param b: b ∈ R
    @param c: c ∈ R
    @return:

    @params
    [a]
    decimals = 5
    step = 0.00005

    [b]
    decimals = 5
    step = 0.00005

    [c]
    decimals = 5
    step = 0.00005
    @end

    """
    if a == 0:
        raise ParameterError(parameter_name="a", message="a cannot be zero!")
    uprint(f"Equation:")
    uprint(f"  {a}x² + {b}x + {c} = 0")
    delta = b**2 - 4 * a * c
    if delta < 0:
        return None
    x1 = (-b + delta**0.5) / (2 * a)
    if delta == 0:
        return x1, x1
    x2 = (-b - delta**0.5) / (2 * a)
    return x1, x2


if __name__ == "__main__":
    window_config = FnExecuteWindowConfig(
        title="Equation Solver",
        icon="calculator.ico",
        execute_button_text="Solve",
        clear_button_text="Clear",
        size=(500, 600),
        show_function_result=True,
        function_result_message="roots: {}",
        default_parameter_group_name="Equation Parameters",
    )

    adapter = GUIAdapter()
    adapter.add(equation_solver_3, window_config=window_config)
    adapter.run(show_select_window=True)
