...
On Windows os,  the GUI may look a little blurry on high-resolution displays. This issue stems from Tkinter’s historical lack of native support for modern high-DPI scaling. To address this issue, PyGUIAdapter integrated a workaround, the original code of which was from [hidpi-tk](https://github.com/Wulian233/hidpi-tk). PyGUIAdapter made some modifications to it. The fundamental principle of this workaround is to enable DPI Awareness for the process, calculate an appropriate scaling factor, and replace the relevant methods of the Tk/Toplevel classes. It is essentially a monkey patch. The following code demonstrates how to enable this workaround:
...
> High DPI support is in a early stage. It needs more tests and may have some issues.



#### Settings GUI

There is a easy way for developers to implement a settings interface.

**1. Defining the Settings Object**
...
Key points:
	...
	- The custom settings class must inherit from the `SettingsBase` class or its subclasses.
 - The class member variables will be treated as configuration fields:
   - variable name not start with "_"
   - an instance of `BaseParameterWidgetConfig`, such as `StringValue`, `IntValue`, `RangedIntValue`,  in this example 

- Implement the `serialize()` and `deserialize()` methods in the custom settings class for serialization/deserialization.

- Override  `__init__(**kwargs)`

  

**2. Displaying the Settings Window**

We usually show the setting window via a menu.
...
If you use a json file for settings, PyGUIAdapterLite provide a simpler base class: `JsonSettingsBase`. This class implements the `serialize()` and `deserialize()` methods and provide `load()` and `save()` methods for reading/writing a JSON file.

Defining `AppSettings` based on `JsonSettingsBase`:
...
A complete example: [examples/settings_example_2.py](https://examples/settings_example_2.py)
...
- **docstring_parser**
  ...
  - Project URL: https://github.com/rr-/docstring_parser
- **hidpi-tk**
  - Purpose：For high dpi auto awareness on Windows.
  - License：Apache License 2.0
  - License File：`licenses/hidpi-tk-LICENSE`
  - Project URL:：https://github.com/Wulian233/hidpi-tk
